...
!(function (a) { typeof define === 'function' && define.amd ? define(['jquery'], a) : a(jQuery); }((a) => {
  a.extend(a.fn, { validate(b) { if (!this.length) return void (b && b.debug && window.console && console.warn("Nothing selected, can't validate, returning nothing.")); let c = a.data(this[0], 'validator'); return c || (this.attr('novalidate', 'novalidate'), c = new a.validator(b, this[0]), a.data(this[0], 'validator', c), c.settings.onsubmit && (this.on('click.validate', ':submit', function (b) { c.settings.submitHandler && (c.submitButton = b.target), a(this).hasClass('cancel') && (c.cancelSubmit = !0), void 0 !== a(this).attr('formnovalidate') && (c.cancelSubmit = !0); }), this.on('submit.validate', (b) => { function d() { let d; let e; return c.settings.submitHandler ? (c.submitButton && (d = a("<input type='hidden'/>").attr('name', c.submitButton.name).val(a(c.submitButton).val()).appendTo(c.currentForm)), e = c.settings.submitHandler.call(c, c.currentForm, b), c.submitButton && d.remove(), void 0 !== e ? e : !1) : !0; } return c.settings.debug && b.preventDefault(), c.cancelSubmit ? (c.cancelSubmit = !1, d()) : c.form() ? c.pendingRequest ? (c.formSubmitted = !0, !1) : d() : (c.focusInvalid(), !1); })), c); }, valid() { let b; let c; let d; return a(this[0]).is('form') ? b = this.validate().form() : (d = [], b = !0, c = a(this[0].form).validate(), this.each(function () { b = c.element(this) && b, d = d.concat(c.errorList); }), c.errorList = d), b; }, rules(b, c) { let d; let e; let f; let g; let h; let i; const j = this[0]; if (b) switch (d = a.data(j.form, 'validator').settings, e = d.rules, f = a.validator.staticRules(j), b) { case 'add': a.extend(f, a.validator.normalizeRule(c)), delete f.messages, e[j.name] = f, c.messages && (d.messages[j.name] = a.extend(d.messages[j.name], c.messages)); break; case 'remove': return c ? (i = {}, a.each(c.split(/\s/), (b, c) => { i[c] = f[c], delete f[c], c === 'required' && a(j).removeAttr('aria-required'); }), i) : (delete e[j.name], f); } return g = a.validator.normalizeRules(a.extend({}, a.validator.classRules(j), a.validator.attributeRules(j), a.validator.dataRules(j), a.validator.staticRules(j)), j), g.required && (h = g.required, delete g.required, g = a.extend({ required: h }, g), a(j).attr('aria-required', 'true')), g.remote && (h = g.remote, delete g.remote, g = a.extend(g, { remote: h })), g; } }), a.extend(a.expr[':'], { blank(b) { return !a.trim(`${a(b).val()}`); }, filled(b) { return !!a.trim(`${a(b).val()}`); }, unchecked(b) { return !a(b).prop('checked'); } }), a.validator = function (b, c) { this.settings = a.extend(!0, {}, a.validator.defaults, b), this.currentForm = c, this.init(); }, a.validator.format = function (b, c) { return arguments.length === 1 ? function () { const c = a.makeArray(arguments); return c.unshift(b), a.validator.format.apply(this, c); } : (arguments.length > 2 && c.constructor !== Array && (c = a.makeArray(arguments).slice(1)), c.constructor !== Array && (c = [c]), a.each(c, (a, c) => { b = b.replace(new RegExp(`\\{${a}\\}`, 'g'), () => c); }), b); }, a.extend(a.validator, {
    defaults: {
    ...
    prototype: {
      ...
      resetForm() { a.fn.resetForm && a(this.currentForm).resetForm(), this.submitted = {}, this.lastElement = null, this.prepareForm(), this.hideErrors(); let b; const c = this.elements().removeData('previousValue').removeAttr('aria-invalid'); if (this.settings.unhighlight) for (b = 0; c[b]; b++) this.settings.unhighlight.call(this, c[b], this.settings.errorClass, ''); else c.removeClass(this.settings.errorClass); },
      numberOfInvalids() { return this.objectLength(this.invalid); },
      objectLength(a) { let b; let c = 0; for (b in a)c++; return c; },
      ...
      elements() {
        const b = this; const c = {}; return a(this.currentForm).find('input, select, textarea').not(':submit, :reset, :image, :disabled').not(this.settings.ignore)
      ...
      elementValue(b) { let c; const d = a(b); const e = b.type; return e === 'radio' || e === 'checkbox' ? this.findByName(b.name).filter(':checked').val() : e === 'number' && typeof b.validity !== 'undefined' ? b.validity.badInput ? !1 : d.val() : (c = d.val(), typeof c === 'string' ? c.replace(/\r/g, '') : c); },
      check(b) { b = this.validationTargetFor(this.clean(b)); let c; let d; let e; const f = a(b).rules(); const g = a.map(f, (a, b) => b).length; let h = !1; const i = this.elementValue(b); for (d in f) { e = { method: d, parameters: f[d] }; try { if (c = a.validator.methods[d].call(this, i, b, e.parameters), c === 'dependency-mismatch' && g === 1) { h = !0; continue; } if (h = !1, c === 'pending') return void (this.toHide = this.toHide.not(this.errorsFor(b))); if (!c) return this.formatAndAdd(b, e), !1; } catch (j) { throw this.settings.debug && window.console && console.log(`Exception occurred when checking element ${b.id}, check the '${e.method}' method.`, j), j instanceof TypeError && (j.message += `.  Exception occurred when checking element ${b.id}, check the '${e.method}' method.`), j; } } if (!h) return this.objectLength(f) && this.successList.push(b), !0; },
      ...
      formatAndAdd(b, c) { let d = this.defaultMessage(b, c.method); const e = /\$?\{(\d+)\}/g; typeof d === 'function' ? d = d.call(this, c.parameters, b) : e.test(d) && (d = a.validator.format(d.replace(e, '{$1}'), c.parameters)), this.errorList.push({ message: d, element: b, method: c.method }), this.errorMap[b.name] = d, this.submitted[b.name] = d; },
      addWrapper(a) { return this.settings.wrapper && (a = a.add(a.parent(this.settings.wrapper))), a; },
      defaultShowErrors() { let a; let b; let c; for (a = 0; this.errorList[a]; a++)c = this.errorList[a], this.settings.highlight && this.settings.highlight.call(this, c.element, this.settings.errorClass, this.settings.validClass), this.showLabel(c.element, c.message); if (this.errorList.length && (this.toShow = this.toShow.add(this.containers)), this.settings.success) for (a = 0; this.successList[a]; a++) this.showLabel(this.successList[a]); if (this.settings.unhighlight) for (a = 0, b = this.validElements(); b[a]; a++) this.settings.unhighlight.call(this, b[a], this.settings.errorClass, this.settings.validClass); this.toHide = this.toHide.not(this.toShow), this.hideErrors(), this.addWrapper(this.toShow).show(); },
      ...
      destroy() { this.resetForm(), a(this.currentForm).off('.validate').removeData('validator'); },
    },
    classRuleSettings: {
    ...
    classRules(b) { const c = {}; const d = a(b).attr('class'); return d && a.each(d.split(' '), function () { this in a.validator.classRuleSettings && a.extend(c, a.validator.classRuleSettings[this]); }), c; },
    normalizeAttributeRule(a, b, c, d) { /min|max/.test(c) && (b === null || /number|range|text/.test(b)) && (d = Number(d), isNaN(d) && (d = void 0)), d || d === 0 ? a[c] = d : b === c && b !== 'range' && (a[c] = !0); },
    attributeRules(b) { let c; let d; const e = {}; const f = a(b); const g = b.getAttribute('type'); for (c in a.validator.methods)c === 'required' ? (d = b.getAttribute(c), d === '' && (d = !0), d = !!d) : d = f.attr(c), this.normalizeAttributeRule(e, g, c, d); return e.maxlength && /-1|2147483647|524288/.test(e.maxlength) && delete e.maxlength, e; },
    dataRules(b) { let c; let d; const e = {}; const f = a(b); const g = b.getAttribute('type'); for (c in a.validator.methods)d = f.data(`rule${c.charAt(0).toUpperCase()}${c.substring(1).toLowerCase()}`), this.normalizeAttributeRule(e, g, c, d); return e; },
    staticRules(b) { let c = {}; const d = a.data(b.form, 'validator'); return d.settings.rules && (c = a.validator.normalizeRule(d.settings.rules[b.name]) || {}), c; },
    ...
    methods: {
      required(b, c, d) { if (!this.depend(d, c)) return 'dependency-mismatch'; if (c.nodeName.toLowerCase() === 'select') { const e = a(c).val(); return e && e.length > 0; } return this.checkable(c) ? this.getLength(b, c) > 0 : b.length > 0; },
      ...
      url(a, b) { return this.optional(b) || /^(?:(?:(?:https?|ftp):)?\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})).?)(?::\d{2,5})?(?:[/?#]\S*)?$/i.test(a); },
      ...
      creditcard(a, b) { if (this.optional(b)) return 'dependency-mismatch'; if (/[^0-9 \-]+/.test(a)) return !1; let c; let d; let e = 0; let f = 0; let g = !1; if (a = a.replace(/\D/g, ''), a.length < 13 || a.length > 19) return !1; for (c = a.length - 1; c >= 0; c--)d = a.charAt(c), f = parseInt(d, 10), g && (f *= 2) > 9 && (f -= 9), e += f, g = !g; return e % 10 === 0; },
      ...
      equalTo(b, c, d) { const e = a(d); return this.settings.onfocusout && e.off('.validate-equalTo').on('blur.validate-equalTo', () => { a(c).valid(); }), b === e.val(); },
      remote(b, c, d) {
        if (this.optional(c)) return 'dependency-mismatch'; let e; let f; const g = this.previousValue(c); return this.settings.messages[c.name] || (this.settings.messages[c.name] = {}), g.originalMessage = this.settings.messages[c.name].remote, this.settings.messages[c.name].remote = g.message, d = typeof d === 'string' && { url: d } || d, g.old === b ? g.valid : (g.old = b, e = this, this.startRequest(c), f = {}, f[c.name] = b, a.ajax(a.extend(!0, {
          mode: 'abort', port: `validate${c.name}`, dataType: 'json', data: f, context: e.currentForm, success(d) { let f; let h; let i; const j = d === !0 || d === 'true'; e.settings.messages[c.name].remote = g.originalMessage, j ? (i = e.formSubmitted, e.prepareElement(c), e.formSubmitted = i, e.successList.push(c), delete e.invalid[c.name], e.showErrors()) : (f = {}, h = d || e.defaultMessage(c, 'remote'), f[c.name] = g.message = a.isFunction(h) ? h(b) : h, e.invalid[c.name] = !0, e.showErrors(f)), g.valid = j, e.stopRequest(c, j); },
        }, d)), 'pending');
      },
    },
  }); let b; const c = {}; a.ajaxPrefilter ? a.ajaxPrefilter((a, b, d) => { const e = a.port; a.mode === 'abort' && (c[e] && c[e].abort(), c[e] = d); }) : (b = a.ajax, a.ajax = function (d) { const e = ('mode' in d ? d : a.ajaxSettings).mode; const f = ('port' in d ? d : a.ajaxSettings).port; return e === 'abort' ? (c[f] && c[f].abort(), c[f] = b.apply(this, arguments), c[f]) : b.apply(this, arguments); });
}));
